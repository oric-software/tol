...
.(
	...
	lda zero_03
	bne skip6
	// Hero - Check quantity of hero-owned projectiles
	sty temp_01
	ldy #00
	ldx #07
loop2	lda projectile_direction,x
	bmi skip5	;skip
	lda projectile_sourcecreature,x
	bne skip5	;Not Hero's
	iny
skip5	dex
	bpl loop2
	cpy #00
	beq skip7
	cpy heroes_projectile_quantitylimit
	bcs skip2	;Total reached
skip7	ldy temp_01
skip6	// Now find spare one
	ldx #07
loop1	lda projectile_direction,x
	bmi skip1
	dex
	bpl loop1
	jmp skip2
skip1	lda zero_00
	...
	;This bit of code ensures that if arrows(126/127), then need to use horiz/vertical
	;But if hero weapon then need to use 95 only (Animations.s deal with anims)
	bcc skip3
	cmp #96
...
skip3	sta projectile_character,x		;95(Hero Special),126(Arrow H),127(Arrow V)
	...
	bne skip4	;Not hero
	lda projectile_property_quantitylimit,y
	sta heroes_projectile_quantitylimit
skip4	lda projectile_property_distancelimiter,y
...
// Projectiles must always be relative to ground, so if the hero moves, we must have routines
// to keep the projectile on course (And not inline with hero).
projectiles_south4hero_north
	ldx #07
.(
loop1	lda projectile_direction,x
	bmi skip1
...
skip2	tya
	...
	lda creature_activity,y
	bpl skip7	;Don't hurt creature if already dying
	...
	lda #25
;	sta sfx_trigger+1
	// Was creature killed a friend?
	lda creature_foe,y
	bne skip7
	// Aargh!, creature was friend, so all friends now turn against the murdering swine!
	lda #128
	sta friend_is_enemy
...
drop_dagger
	ldy projectile_y,x
	lda projectile_x,x
	// convert Daggers x/y to bigmap coords
	pha
	;Fetch tl hero screen
	lda hero_xl
	sec
	sbc #17
	sta test_xl
...
skip2
.)
	; Now store to last entry in ground_items (+11)
	lda test_xl
	sta ground_item_xl+11
	lda test_xh
	sta ground_item_xh+11
	lda test_yl
	sta ground_item_yl+11
	lda test_yh
	sta ground_item_yh+11
	lda #118	;The ground item character is a weapon
	sta ground_item_character+11	;If 0 then not present
	lda #183	;The ground item textnum is offcourse the dagger!
	sta ground_item_textnum+11
	rts
