...
#define	PTR_SELECTCONTROLLER	0	;Needs defining
#define	PTR_RETURNTOMAINMENU	0	;Needs defining

#define	PTR_STARTNEWGAME		0
#define	PTR_RETURNTOTIMESOFLORE	1
#define	PTR_OPTIONS		2
#define	PTR_MAPANDSTATS		3
#define	PTR_SELECTVOLUME		4
#define	PTR_SELECTDIFFICULTY	5
#define	PTR_WARRIOR		6
#define	PTR_PARTISAN		7
#define	PTR_PEASANT		8
...
;	jmp test999
	...
	jsr DitherInCharset
;	jsr SetupOptionsGloveXPos

  jsr tn_copychrtable ; Tobias
	
	lda MenuRowsGloveX
	sta GloveDestinationX
	lda MenuRowsGloveY
	sta GloveDestinationY
	lda #00
	sta GloveX
	sta GloveY
	lda #1
	sta GloveMovingFlag
	sta BackgroundRestoredFlag		
...
.)
MainLoopSkip
	jmp JumpOutAndDisplayStaticMap2

SelectedOptionFlag
GloveMovingFlag	.byt 1
GloveActiveFlag	.byt 0
OldKey		.byt 0
BackgroundRestoredFlag	.byt 0
OptionReached
	lda #00
	sta GloveMovingFlag
	rts
MoveGlove
	;First check if both X/Y have been reached
	lda GloveX
	cmp GloveDestinationX
.(
	bne skip1
	lda GloveY
	cmp GloveDestinationY
	beq OptionReached
skip1	;Store current screen loc
.)
	ldx GloveX
	ldy GloveY
	lda ScreenXLOC,x
...
.(
	...
	jmp skip2
skip1	dec GloveY
skip2	;calculate Screen loc again
.)
	ldx GloveX
	ldy GloveY
	lda ScreenXLOC,x
...
.(
	bne skip2
	lda GloveScreen+1
	cmp OldGloveScreen+1
	beq skip1
skip2	;Restore bg if different
	jsr RestoreGloveBackground
	lda #1
	sta BackgroundRestoredFlag	
skip1	jmp PlotGloveAtXY
.)
...
.(
	...
	dec SelectedOption
	ldx SelectedOption
	lda MenuRowsGloveX,x
	sta GloveDestinationX
	lda MenuRowsGloveY,x
	sta GloveDestinationY
	lda #1
	sta GloveMovingFlag
skip1	rts
...
SelectOption
	...
	beq so_StartNewGame
	cmp #2
	bcc so_ReturnToTimesOfLore
	beq so_Options
	cmp #4
	bcc so_MapAndStats
	cmp #7
	bcc so_ControllerSelection
	cmp #9
	bcc so_AudioSelection
	cmp #12
	bcc so_DifficultySelection
	beq so_ReturnToMainMenu
	cmp #14
	bcc so_ViewCurrentLocation
	beq so_ScoutAlbareth
	jmp so_ViewStatistics

so_ViewCurrentLocation
so_ScoutAlbareth
so_ViewStatistics
so_StartNewGame
so_ReturnToTimesOfLore
	rts

so_ControllerSelection
	ldx CurrentController
	inx
	cpx #3
.(
	bcc skip1
	ldx #0
skip1	stx CurrentController
.)
	clc
	txa
	adc #4
	sta MenuList1
so_CSRent	jsr PlotMenu
	lda #1
	sta BackgroundRestoredFlag
	jmp PlotGloveAtXY
...
so_DifficultySelection
	ldx CurrentDifficulty
	inx
	cpx #3
.(
	bcc skip1
	ldx #0
skip1	stx CurrentDifficulty
.)
	clc
	txa
	adc #9
...
loop1	lda DisplayMapFlag
	bne JumpOutAndDisplayStaticMap
	lda IRQDelay
	bne loop1
	rts
JumpOutAndDisplayStaticMap
.)
	pla
	pla
JumpOutAndDisplayStaticMap2
	;Display map
	ldx #04
	jsr Depack2Screen
	jsr InvertTarget
	;Wait on DisplayMapFlag
.(
loop1	lda DisplayMapFlag
	bne loop1
.)
...
.(
	beq skip1
	;Delay glove management by 4 irq's
	lda GloveDelayFrac
	adc #128
	sta GloveDelayFrac
	bcc skip1
...
ScreenOffsetHi
 ...
 .byt >40*118
 .byt >40*119
;This routine should have replaced the above tables but i can't get it working right (x40) atm
;	sta TempX
;	;Multiply YPOS by 8
;	lda #00
;	sta Mult8+1
...
;	rol Mult8+1
;	sta Mult8
;	;Shift Result twice more to get x32
;	asl
;	rol Mult32Hi
;	asl
;	rol Mult32Hi
;	;Add last calc to get x40
;	adc Mult8
;	tay
;	lda Mult32Hi
;	adc Mult8+1
...
;$B8D2-$B8FF Spare
;$B900-$B99F Glove Mask Buffer(5x32)
;$B9A0-$BDD7 Spare
SpareBit2
 .dsb $BDD8-*
